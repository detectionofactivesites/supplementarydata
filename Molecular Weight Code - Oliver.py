...
def calculate_protein_molecular_weight(af_model):
    molecular_weight = 0.0
    for model in af_model:
        for chain in model:
            for residue in chain:
                residue_name = gemmi.find_tabulated_residue(residue.name)
                if residue_name is not None:
                    molecular_weight += residue_name.weight
...
def find_structural_motifs ( filename = "",
  ...
  if len ( result_list ) > 0 :
    ...
    protein_molecular_weight = calculate_protein_molecular_weight(af_model)
    print(f"\nOverall Molecular Weight of the Protein: {protein_molecular_weight:.2f} g/mol\n")
  else :
    print ("\nNo results found :-( \n")
  return result_dict
