...
def Retrieving_ACC(filename):
   # Define the URL and parameters
   url = "https://pdb-redo.eu/dssp/do"
   # Specify the path to the PDB file you want to upload
   pdb_file_path = filename
   # Specify the desired format (mmcif or dssp)
   format_value = "dssp"


   # Prepare the file and format data
   files = {"data": open(pdb_file_path, "rb")}
   ...
   if response.status_code == 200:
       ...
       dssp_content = response.content
       dssp_file_path = f"{filename[:len(filename) - 4]}.dssp"  # Name the file according to your preferences
       with open(dssp_file_path, "wb") as f:
           f.write(dssp_content)


       # Initialize a list to store ACC values
       acc_values = []


       # Adding ACC extraction
       with open(dssp_file_path, "r") as f:
           # Flag to indicate whether to start extracting ACC values
           start_extracting = False
           # Iterate over each line in the file
           for line in f:
               # Check if the line starts with 'RESIDUE' (indicating start of data lines)
               if line.startswith("  #  RESIDUE"):
                   # Start extracting ACC values
                   start_extracting = True
                   continue
               # Check if the line starts with ' ' (indicating data lines) and extraction flag is True
               elif line.startswith(" ") and start_extracting:
                   # Extract the ACC value from the line
                   acc_value_str = line[34:40].strip()
                   # Attempt to convert the ACC value to float
                   try:
                       acc_value = float(acc_value_str)
                       # Append the ACC value to the list
                       acc_values.append(acc_value)
                   except ValueError:
                       # If conversion fails, print a message and continue to the next line
                       print("Failed to convert ACC value to float:", acc_value_str)
                       continue
               # Check if the line does not start with ' ' (indicating end of data lines) and extraction flag is True
               elif not line.startswith(" ") and start_extracting:
                   # Stop extracting ACC values
                   start_extracting = False


       # Print the list of ACC values
       return acc_values
...
def find_structural_motifs(filename="",
   ...
   if len(result_list) > 0:
       ...
       for result in result_list:
           ...
           hit_list.append(hit)
           # Print Hit elements
           ACC = Retrieving_ACC(filename)
           print("Hit Found:")
           for residue in hit:
               print(
                   f"Residue:{residue['name']}, seqid:{residue['seqid']}, plddt:{residue['plddt']}, coordinates: {residue['coordinates']}, SA: {ACC[int(residue['seqid']) - 1]}")
