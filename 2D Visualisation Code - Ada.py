...
def find_structural_motifs(filename="", residue_lists=[], distance=0.0, min_plddt=70.0, n_term=False, c_term=False):
...
def display_hits_gui(result_list):
    for result in result_list:
        for residue in result:
            plt.scatter(residue[-1].pos.x, residue[-1].pos.y)
            plt.text(residue[-1].pos.x, residue[-1].pos.y + 0.1, f"{residue.name}{residue.seqid.num}",
                     horizontalalignment='center')
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    plt.title('Hits Visualization')
    plt.show()
